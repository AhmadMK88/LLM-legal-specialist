...
def normalize_schema(dataset: Dataset) -> Dataset:
    """
...
def format_example(example :Dict , eos_token: str) -> Dict:
    ...
    formatted_example = {"text": prompt + answer + eos_token.eos_token}

    return formatted_example


def tokenize(examples: Dict, tokenizer: PreTrainedTokenizerBase) -> Dict:
    """
...
def _load_datasets(split: str) -> Dataset:
    ...
    for path, name in DATASET_CONFIGS:
        ...
        normalized_dataset = normalize_schema(dataset[split])
...
def create_dataset(tokenizer: PreTrainedTokenizerBase) -> Tuple:
    ...
    # Preprocess dataset
    train_dataset = train_dataset.map(lambda x: format_example(x, tokenizer))
    test_dataset = test_dataset.map(lambda x: format_example(x, tokenizer))
